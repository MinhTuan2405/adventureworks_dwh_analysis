{{ 
  config(
    ...
    file_format = 'parquet',
    tags =['bronze'],
    location = 's3://lakehouse/bronze/base_purchase_order_header.parquet'
  ) 
}}

with source as (
    select *
    from read_parquet('s3://lakehouse/landing/purchasing/purchasing_purchaseorderheader.parquet')
)
