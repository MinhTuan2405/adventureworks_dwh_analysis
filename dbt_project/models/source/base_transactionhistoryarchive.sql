{{ 
  config(
    ...
    location = 's3://lakehouse/bronze/base_transactionhistoryarchive.parquet'
  ) 
}}

with source as (
    select *
    from read_parquet('s3://lakehouse/landing/production/production_transactionhistoryarchive.parquet')
),
