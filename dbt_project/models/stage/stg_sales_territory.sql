{{ 
  config(
    ...
    file_format = 'parquet',
    tags = [ 'staging'],
    location = 's3://lakehouse/silver/stg_sales_territory.parquet'
  ) 
}}

with sales_territory as (
    select * 
    from {{ ref('base_sales_territory') }}
)
...
, joined as (
    select
        -- Surrogate key
        {{ dbt_utils.generate_surrogate_key(['st.territory_id']) }} as dim_sales_territory_sk,
