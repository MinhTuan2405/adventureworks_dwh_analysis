...
dim_location as (
    select location_id, dim_adventureworks_location_sk
...
final as (
    select
        -- Surrogate Key
        {{ dbt_utils.generate_surrogate_key(['i.product_id', 'i.location_id']) }} as fct_adventureworks_inventory_sk,
        
        -- Foreign Keys
        coalesce(p.dim_adventureworks_product_sk, '-1') as dim_adventureworks_product_sk,
        coalesce(l.dim_adventureworks_location_sk, '-1') as dim_adventureworks_location_sk,
