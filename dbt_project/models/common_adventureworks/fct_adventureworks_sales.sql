{{ 
  config(
    ...
    location = 's3://lakehouse/warehouse/adventureworks/fct_adventureworks_sales.parquet'
  ) 
}}
...
, final as (
    select
        -- Primary key
        {{ dbt_utils.generate_surrogate_key(['sales_order_detail_id']) }} as adventureworks_sales_pk,
