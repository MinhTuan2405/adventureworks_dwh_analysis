...
with order_header as (
    ...
    where is_online_order = false  -- Filter for reseller orders only
)
...
, joined as (
    select
        ...
        h.customer_id,
        c.store_id as reseller_id,  -- Store represents reseller in this context
        d.product_id,
        d.special_offer_id,
        h.sales_person_id,
        h.territory_id,
...
, dim_reseller as (
    select 
        dim_adventureworks_reseller_sk,
        store_id
    from {{ ref('dim_adventureworks_reseller') }}
)
...
, final as (
    select
        -- Primary Key
        {{ dbt_utils.generate_surrogate_key(['j.sales_order_detail_id']) }} as fct_adventureworks_reseller_sales_pk,
        
        -- Foreign Keys 
        coalesce(dc.dim_adventureworks_customer_sk, {{ dbt_utils.generate_surrogate_key(["'-1'"]) }}) as dim_adventureworks_customer_sk,
        coalesce(dr.dim_adventureworks_reseller_sk, {{ dbt_utils.generate_surrogate_key(["'-1'"]) }}) as dim_adventureworks_reseller_sk,
    ...
    left join dim_reseller dr on j.reseller_id = dr.store_id
